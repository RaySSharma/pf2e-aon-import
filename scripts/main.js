    ...
    controls.tokens.tools.aonimport = {
      ...
      onChange: async () => {
        try {
          ...
          const result = await window.AoNImport.openImportDialog({
            compile: true,
            retrieveDocument: false,
          });
          if (result)
            ui.notifications?.info(
              `AoN Import: parsed ${result.parsed.length} items`
            );
...
Hooks.once("ready", async function () {
  ...
  try {
    ...
    console.warn(
      `${MODULE_ID} | module object not found; AoNImport available at window.AoNImport`
    );
  } catch (err) {
